#!/usr/bin/env python3
"""
GitHub Fabric Activator

Handles GitHub operations via MCP server.
...
# Configuration
FABRIC_NAME = os.getenv("FABRIC_NAME", "github")
REDIS_HOST = os.getenv("REDIS_HOST", "redis.cortex-chat.svc.cluster.local")
REDIS_PORT = int(os.getenv("REDIS_PORT", "6379"))
REDIS_PASSWORD = os.getenv("REDIS_PASSWORD", "")
TASK_STREAM = os.getenv("TASK_STREAM", "cortex.github.tasks")
RESULT_STREAM = os.getenv("RESULT_STREAM", "cortex.results")
CONSUMER_GROUP = os.getenv("CONSUMER_GROUP", "github-activator")
...
async def load_mcp_config():
    ...
    try:
        ...
        # Use defaults
        mcp_servers = {
            "github-mcp": "http://github-mcp-server.cortex-system.svc.cluster.local:3002",
        }
...
async def process_query(query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    if "repo" in query_lower or "repository" in query_lower:
        ...
        result = {"message": "Code search requires a search query. Example: 'search code for redis client'"}
    else:
        # Default: list available capabilities
        result = {
            "message": f"GitHub fabric ready. Available tools: {len(mcp_tools)}",
            "capabilities": [
                "List repositories",
...
async def lifespan(app: FastAPI):
    ...
    logger.info("github_activator_starting", fabric=FABRIC_NAME)
    ...
    logger.info("github_activator_ready", tools=len(mcp_tools))

    yield

    # Shutdown
    logger.info("github_activator_stopping")
    running = False
