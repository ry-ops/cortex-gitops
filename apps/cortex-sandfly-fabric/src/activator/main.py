#!/usr/bin/env python3
"""
Sandfly Security Fabric Activator

Handles security scanning and threat detection via MCP server.
...
# Configuration
FABRIC_NAME = os.getenv("FABRIC_NAME", "sandfly")
REDIS_HOST = os.getenv("REDIS_HOST", "redis.cortex-chat.svc.cluster.local")
REDIS_PORT = int(os.getenv("REDIS_PORT", "6379"))
REDIS_PASSWORD = os.getenv("REDIS_PASSWORD", "")
TASK_STREAM = os.getenv("TASK_STREAM", "cortex.sandfly.tasks")
RESULT_STREAM = os.getenv("RESULT_STREAM", "cortex.results")
CONSUMER_GROUP = os.getenv("CONSUMER_GROUP", "sandfly-activator")
...
async def process_query(query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    # Simple keyword routing to Sandfly tools
    if "host" in query_lower:
        if "list" in query_lower:
            result = await call_tool("sandfly-mcp__list_hosts", {})
            tool_calls = 1
        elif "scan" in query_lower:
            result = {"message": "Host scanning requires: hostname or IP address"}
        else:
            result = await call_tool("sandfly-mcp__list_hosts", {})
            tool_calls = 1
    elif "alert" in query_lower or "threat" in query_lower:
        if "list" in query_lower or "show" in query_lower:
            result = await call_tool("sandfly-mcp__list_alerts", {})
            tool_calls = 1
        else:
            result = await call_tool("sandfly-mcp__list_alerts", {"status": "active"})
            tool_calls = 1
    elif "scan" in query_lower:
        if "status" in query_lower:
            result = await call_tool("sandfly-mcp__get_scan_status", {})
            tool_calls = 1
        elif "start" in query_lower or "run" in query_lower:
            result = {"message": "Scan requires: target host or group"}
        else:
            result = await call_tool("sandfly-mcp__list_scans", {})
            tool_calls = 1
    elif "vulnerability" in query_lower or "vuln" in query_lower:
        result = await call_tool("sandfly-mcp__list_vulnerabilities", {})
        tool_calls = 1
    elif "compliance" in query_lower:
        result = await call_tool("sandfly-mcp__get_compliance_status", {})
        tool_calls = 1
    elif "security" in query_lower and "status" in query_lower:
        result = await call_tool("sandfly-mcp__get_security_summary", {})
        tool_calls = 1
    else:
        # Default: list available capabilities
        result = {
            "message": f"Sandfly security fabric ready. Available tools: {len(mcp_tools)}",
            "capabilities": [
                "List/scan hosts",
...
async def lifespan(app: FastAPI):
    ...
    logger.info("sandfly_activator_starting", fabric=FABRIC_NAME)
    ...
    logger.info("sandfly_activator_ready", tools=len(mcp_tools))

    yield

    logger.info("sandfly_activator_stopping")
    running = False
...
app = FastAPI(
    title="Sandfly Security Fabric Activator",
    description="Security scanning via MCP",
    version="1.0.0",
    lifespan=lifespan
)
