#!/usr/bin/env python3
"""
Proxmox Fabric Activator

Handles Proxmox VM and container operations via MCP server.
...
# Configuration
FABRIC_NAME = os.getenv("FABRIC_NAME", "proxmox")
REDIS_HOST = os.getenv("REDIS_HOST", "redis.cortex-chat.svc.cluster.local")
REDIS_PORT = int(os.getenv("REDIS_PORT", "6379"))
REDIS_PASSWORD = os.getenv("REDIS_PASSWORD", "")
TASK_STREAM = os.getenv("TASK_STREAM", "cortex.proxmox.tasks")
RESULT_STREAM = os.getenv("RESULT_STREAM", "cortex.results")
CONSUMER_GROUP = os.getenv("CONSUMER_GROUP", "proxmox-activator")
...
async def process_query(query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    # Simple keyword routing to Proxmox tools
    if "vm" in query_lower or "virtual machine" in query_lower:
        if "list" in query_lower or "show" in query_lower:
            ...
            tool_calls = 1
        elif "start" in query_lower:
            result = {"message": "VM start requires: node and vmid"}
        elif "stop" in query_lower:
            result = {"message": "VM stop requires: node and vmid"}
        elif "create" in query_lower:
            result = {"message": "VM creation requires: node, vmid, name, and resources"}
        else:
            result = await call_tool("proxmox-mcp__list_vms", {})
            tool_calls = 1
    elif "container" in query_lower or "lxc" in query_lower:
        if "list" in query_lower:
            result = await call_tool("proxmox-mcp__list_containers", {})
            tool_calls = 1
        elif "create" in query_lower:
            result = {"message": "Container creation requires: node, vmid, template, and resources"}
        else:
            result = await call_tool("proxmox-mcp__list_containers", {})
            tool_calls = 1
        ...
        result = await call_tool("proxmox-mcp__list_storage", {})
        tool_calls = 1
        ...
        if "list" in query_lower:
            result = await call_tool("proxmox-mcp__list_backups", {})
            tool_calls = 1
        else:
            result = {"message": "Backup operations: list, create, restore"}
    else:
        # Default: list available capabilities
        result = {
            "message": f"Proxmox fabric ready. Available tools: {len(mcp_tools)}",
            "capabilities": [
                "List/manage VMs",
                "List/manage LXC containers",
                "List nodes",
                "Storage management",
                "Cluster status",
                "Backup operations"
...
async def lifespan(app: FastAPI):
    ...
    logger.info("proxmox_activator_starting", fabric=FABRIC_NAME)
    ...
    logger.info("proxmox_activator_ready", tools=len(mcp_tools))

    yield

    logger.info("proxmox_activator_stopping")
    running = False
...
app = FastAPI(
    title="Proxmox Fabric Activator",
    description="Proxmox VM/container operations via MCP",
    version="1.0.0",
    lifespan=lifespan
)
