#!/usr/bin/env python3
"""
Cloudflare Fabric Activator

Handles Cloudflare operations via MCP server.
...
# Configuration
FABRIC_NAME = os.getenv("FABRIC_NAME", "cloudflare")
REDIS_HOST = os.getenv("REDIS_HOST", "redis.cortex-chat.svc.cluster.local")
REDIS_PORT = int(os.getenv("REDIS_PORT", "6379"))
REDIS_PASSWORD = os.getenv("REDIS_PASSWORD", "")
TASK_STREAM = os.getenv("TASK_STREAM", "cortex.cloudflare.tasks")
RESULT_STREAM = os.getenv("RESULT_STREAM", "cortex.results")
CONSUMER_GROUP = os.getenv("CONSUMER_GROUP", "cloudflare-activator")
...
async def load_mcp_config():
    ...
    try:
        ...
        mcp_servers = {
            "cloudflare-mcp": "http://cloudflare-mcp-server.cortex-system.svc.cluster.local:3000",
        }
...
async def process_query(query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    # Simple keyword routing to Cloudflare tools
    if "dns" in query_lower or "record" in query_lower:
        if "list" in query_lower:
            result = await call_tool("cloudflare-mcp__list_dns_records", {"zone_id": "auto"})
            tool_calls = 1
        elif "create" in query_lower or "add" in query_lower:
            result = {"message": "DNS record creation requires: type (A, CNAME, etc), name, content"}
        else:
            result = await call_tool("cloudflare-mcp__list_dns_records", {"zone_id": "auto"})
            tool_calls = 1
    elif "zone" in query_lower:
        if "list" in query_lower:
            result = await call_tool("cloudflare-mcp__list_zones", {})
            tool_calls = 1
        else:
            result = await call_tool("cloudflare-mcp__list_zones", {})
            tool_calls = 1
    elif "tunnel" in query_lower:
        if "list" in query_lower:
            result = await call_tool("cloudflare-mcp__list_tunnels", {})
            tool_calls = 1
        elif "create" in query_lower:
            result = {"message": "Tunnel creation requires: name"}
        else:
            result = await call_tool("cloudflare-mcp__list_tunnels", {})
            tool_calls = 1
    elif "waf" in query_lower or "firewall" in query_lower:
        if "rule" in query_lower:
            result = await call_tool("cloudflare-mcp__list_firewall_rules", {"zone_id": "auto"})
            tool_calls = 1
        else:
            result = {"message": "WAF operations: list rules, create rule, enable/disable"}
    elif "analytics" in query_lower or "traffic" in query_lower:
        result = await call_tool("cloudflare-mcp__get_analytics", {"zone_id": "auto"})
        tool_calls = 1
    elif "cache" in query_lower:
        if "purge" in query_lower:
            result = {"message": "Cache purge requires confirmation. Specify: all or specific URLs"}
        else:
            result = {"message": "Cache operations: purge all, purge URLs, cache settings"}
    else:
        # Default: list available capabilities
        result = {
            "message": f"Cloudflare fabric ready. Available tools: {len(mcp_tools)}",
            "capabilities": [
                "List/manage DNS records",
                "List zones",
                "List/manage tunnels",
                "WAF/firewall rules",
...
async def lifespan(app: FastAPI):
    ...
    logger.info("cloudflare_activator_starting", fabric=FABRIC_NAME)
    ...
    logger.info("cloudflare_activator_ready", tools=len(mcp_tools))

    yield

    logger.info("cloudflare_activator_stopping")
    running = False
...
app = FastAPI(
    title="Cloudflare Fabric Activator",
    description="Cloudflare operations via MCP",
    version="1.0.0",
    lifespan=lifespan
)
