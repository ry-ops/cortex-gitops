#!/usr/bin/env python3
"""
Infrastructure Fabric Activator

Handles Proxmox and Kubernetes operations via MCP servers.
...
# Configuration
FABRIC_NAME = os.getenv("FABRIC_NAME", "infrastructure")
REDIS_HOST = os.getenv("REDIS_HOST", "redis.cortex-system.svc.cluster.local")
REDIS_PORT = int(os.getenv("REDIS_PORT", "6379"))
REDIS_PASSWORD = os.getenv("REDIS_PASSWORD", "")
TASK_STREAM = os.getenv("TASK_STREAM", "cortex.infra.tasks")
RESULT_STREAM = os.getenv("RESULT_STREAM", "cortex.results")
CONSUMER_GROUP = os.getenv("CONSUMER_GROUP", "infra-activator")
...
async def process_query(query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    # Simple keyword routing to tools
    if "vm" in query_lower or "virtual machine" in query_lower:
        if "list" in query_lower or "show" in query_lower:
            result = await call_tool("proxmox-mcp__list_vms", {})
            tool_calls = 1
        elif "create" in query_lower:
            result = {"message": "VM creation requires specific parameters"}
        else:
            result = await call_tool("proxmox-mcp__list_vms", {})
            tool_calls = 1
        ...
        if "kubernetes" in query_lower or "k8s" in query_lower:
            ...
            result = await call_tool("proxmox-mcp__list_nodes", {})
            tool_calls = 1
    elif "pod" in query_lower:
        result = await call_tool("kubernetes-mcp__list_pods", {"namespace": "default"})
        tool_calls = 1
    elif "deployment" in query_lower:
        result = await call_tool("kubernetes-mcp__list_deployments", {"namespace": "default"})
        tool_calls = 1
    elif "container" in query_lower and "proxmox" in query_lower:
        result = await call_tool("proxmox-mcp__list_containers", {})
        tool_calls = 1
    elif "cluster" in query_lower:
        if "proxmox" in query_lower:
            result = await call_tool("proxmox-mcp__get_cluster_status", {})
        else:
        ...
        result = {
            "message": f"Infrastructure fabric ready. Available tools: {len(mcp_tools)}",
            "capabilities": [
                "List VMs (proxmox)",
                "List containers (proxmox)",
                "List nodes (proxmox/kubernetes)",
                "List pods (kubernetes)",
                "List deployments (kubernetes)",
                "Cluster status"
            ]
        }
...
async def lifespan(app: FastAPI):
    ...
    logger.info("infra_activator_starting", fabric=FABRIC_NAME)
    ...
    logger.info("infra_activator_ready", tools=len(mcp_tools))

    yield

    # Shutdown
    logger.info("infra_activator_stopping")
    running = False
...
app = FastAPI(
    title="Infrastructure Fabric Activator",
    description="Proxmox and Kubernetes operations via MCP",
    version="1.0.0",
    lifespan=lifespan
)
