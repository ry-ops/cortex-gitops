#!/usr/bin/env python3
"""
n8n Fabric Activator

Handles workflow automation operations via n8n MCP server.
...
# Configuration
FABRIC_NAME = os.getenv("FABRIC_NAME", "n8n")
REDIS_HOST = os.getenv("REDIS_HOST", "redis.cortex-chat.svc.cluster.local")
REDIS_PORT = int(os.getenv("REDIS_PORT", "6379"))
REDIS_PASSWORD = os.getenv("REDIS_PASSWORD", "")
TASK_STREAM = os.getenv("TASK_STREAM", "cortex.n8n.tasks")
RESULT_STREAM = os.getenv("RESULT_STREAM", "cortex.results")
CONSUMER_GROUP = os.getenv("CONSUMER_GROUP", "n8n-activator")
MCP_SERVER_URL = os.getenv("MCP_SERVER_URL", "http://n8n-mcp-server.cortex-system.svc.cluster.local:3002")
...
async def discover_tools():
    """Discover available tools from n8n MCP server."""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for tool in tools:
                    ...
                    prefixed_name = f"n8n-mcp__{tool_name}"
                    ...
                    mcp_tools.append({
                        "name": prefixed_name,
                        "description": f"[n8n] {tool.get('description', '')}",
                        "input_schema": tool.get("inputSchema", {})
                    })
                logger.info("mcp_tools_discovered", server="n8n-mcp", count=len(tools))
        except Exception as e:
            logger.error("mcp_discovery_error", server="n8n-mcp", error=str(e))

    logger.info("mcp_discovery_complete", total_tools=len(mcp_tools))


async def call_tool(tool_name: str, arguments: Dict[str, Any]) -> Any:
    """Execute a tool on n8n MCP server."""
...
async def process_query(query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
    """Process a query using n8n MCP tools."""
    start = time.time()
    tool_calls = 0
    query_lower = query.lower()

    # Simple keyword routing to n8n tools
    if "workflow" in query_lower:
        if "list" in query_lower or "show" in query_lower or "all" in query_lower:
            result = await call_tool("n8n-mcp__list_workflows", {})
            tool_calls = 1
        elif "execute" in query_lower or "run" in query_lower or "trigger" in query_lower:
            result = {"message": "To execute a workflow, I need the workflow ID. Use 'list workflows' first to see available workflows."}
        elif "create" in query_lower:
            result = {"message": "Workflow creation requires: name, nodes configuration, and connections."}
        else:
            result = await call_tool("n8n-mcp__list_workflows", {})
            tool_calls = 1
    elif "execution" in query_lower:
        if "list" in query_lower or "history" in query_lower:
            result = await call_tool("n8n-mcp__list_executions", {})
            tool_calls = 1
        else:
            result = await call_tool("n8n-mcp__list_executions", {"limit": 10})
            tool_calls = 1
    elif "credential" in query_lower:
        if "list" in query_lower:
            result = await call_tool("n8n-mcp__list_credentials", {})
            tool_calls = 1
        else:
            result = {"message": "Credential management: Use 'list credentials' to see available credentials."}
    elif "automation" in query_lower or "automate" in query_lower:
        result = {
            "message": "n8n automation fabric ready. I can help you with:",
            "capabilities": [
                "List workflows",
            ...
            "available_tools": [t["name"].replace("n8n-mcp__", "") for t in mcp_tools]
        }
    else:
        # Default: show capabilities
        result = {
            "message": f"n8n automation fabric ready. Available tools: {len(mcp_tools)}",
            "capabilities": [
                "List workflows",
                "Execute workflows",
                "View execution history",
...
async def lifespan(app: FastAPI):
    ...
    logger.info("n8n_activator_starting", fabric=FABRIC_NAME)
    ...
    logger.info("n8n_activator_ready", tools=len(mcp_tools))

    yield

    # Shutdown
    logger.info("n8n_activator_stopping")
    running = False
...
app = FastAPI(
    title="n8n Fabric Activator",
    description="Workflow automation operations via n8n MCP",
    version="1.0.0",
    lifespan=lifespan
)
