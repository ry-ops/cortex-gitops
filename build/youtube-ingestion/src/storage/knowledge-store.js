...
export class KnowledgeStore {
  ...
  /**
   * Get recent learnings from last N days
   * @param {number} days - Number of days to look back (default: 1)
   * @returns {Promise<Array>} Learnings with formatted data
   */
  async getRecentLearnings(days = 1) {
    const cutoffTime = Date.now() - (days * 24 * 60 * 60 * 1000);

    if (this.redis) {
      // Query Redis for videos ingested since cutoff
      const videoIds = await this.redis.zrangebyscore('youtube:by-date', cutoffTime, Date.now());

      const learnings = [];
      for (const videoId of videoIds) {
        const knowledge = await this.retrieve(videoId);
        if (knowledge) {
          learnings.push(this.formatLearning(knowledge));
        }
      }

      // Sort by ingestion date (newest first)
      learnings.sort((a, b) => new Date(b.ingested_at) - new Date(a.ingested_at));
    ...
    for (const file of files) {
      ...
      const ingestedAt = new Date(knowledge.ingested_at);
      if (ingestedAt.getTime() >= cutoffTime) {
        learnings.push(this.formatLearning(knowledge));
      }
    }
  ...
  /**
   * Get aggregated summary of learnings
   * @returns {Promise<Object>} Summary with categories and highlights
   */
  async getLearningsSummary() {
    const recentLearnings = await this.getRecentLearnings(1);

    if (recentLearnings.length === 0) {
      return {
        total: 0,
        message: 'No new learnings today',
        categories: {},
        highlights: []
      };
    }
    ...
    for (const learning of recentLearnings) {
      ...
      byCategory[category].videos.push({
        title: learning.title,
        video_id: learning.video_id,
        relevance: learning.relevance_to_cortex
      });

      // Add high-relevance videos to highlights
      if (learning.relevance_to_cortex >= 0.8) {
        highlights.push({
          title: learning.title,
          video_id: learning.video_id,
          category: learning.category,
          relevance: learning.relevance_to_cortex,
          key_points: learning.key_points?.slice(0, 3) || []
        });
      }
    }
    ...
    return {
      total: recentLearnings.length,
      message: `Learned from ${recentLearnings.length} video${recentLearnings.length !== 1 ? 's' : ''} today`,
      categories: byCategory,
      highlights: highlights.slice(0, 5), // Top 5 highlights
      all_learnings: recentLearnings
    };
  }

  /**
   * Format knowledge object into a learning entry
   * @private
   * @param {Object} knowledge - Full knowledge object
   * @returns {Object} Formatted learning entry
   */
  formatLearning(knowledge) {
    return {
      video_id: knowledge.video_id,
      title: knowledge.title,
      category: knowledge.category,
      relevance_to_cortex: knowledge.relevance_to_cortex,
      ingested_at: knowledge.ingested_at,
      key_points: knowledge.key_points || [],
      tags: knowledge.tags || [],
      url: `https://www.youtube.com/watch?v=${knowledge.video_id}`
    };
  }
}
